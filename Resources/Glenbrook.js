...
var Goblin_Warlord = function(vic){
    ...
    gw.performCombat = function(){
        ...
        if(choice === 1){
            game.print("The Goblin Warlord chugs something from his pouch, and you notice his muscles noticably pop...");
            me.buff = 2;
        }
        else if(choice < 5){
...
var i_fireboltUpgrade = Tools.makeItem("\"Advanced Firebolts\"", 25, "This book seems to detail ways more amature casters \
fail to bring out firebolts full potential. While you are <i>sure</i> that you don't make any such mistakes... perhaps... \
it wouldn't hurt to look...?",
function(){
    if(player.gold < 25){
        game.print("The clerk informs you that if you cannot <i>count</i> properly, you definately couldn't use this\
         properly");
         return;
    }
    player.gold -= 25;
...
... enthusiasts. It details way to improve your \"Buff\" spell",
function(){
    if(player.gold < 10){
...
Tools.makeProcess("Glen_Encounter",function(){
    ...
    if(choice < 3){
        ...
        switchLocation("Its the Goblin Warlord!");
    }
    else{
...
Tools.makeEnemyEncounter("Its the Goblin Warlord!", "Its him! Its the goblin warlorf the people in the tavern were talking \
about! Unlike other goblins this one, while small, is impressivly built, with a <i>huge</i> axe. But that doesn't matter \
to you anymore. After all, he will clearly be carrying treasure on him, and ther reward for him back at town isn't small either.",
Goblin_Warlord(function(){
    ...
    over. A pouch from his belt falls off, some gold pieces spilling forth. That will do nicely alongside with \
...
Tools.makeProcess("Glen_Bounty_Collect",function(){
    ...
    this, you may be able to take on that Orc Chief you here is in the mountains!");
...
mention something about there being an <em>Golbin Warlord</em> somewhere in the woods. Supposidly there is \
a pretty big bounty on it... And more! Up ahead in the mountain trail, there is talk of the Orcs having a new chief, you \
imagine that it would increase your reputation a lot to take them down. Although, perhaps some perperations would be \
...
perfectly with the cielings and floors. There are many scrolls, quires and loose papers placed about the shelves and few \
desks. You note with some disapointment that there are actually not too many books here. It only makes sense; paper is \
expensive, and for you to need a books worth of it for anything particular rare.</p><p>It seems that they <i>do</i> \
...
Well, crap. You guess you'll put this into practice!", false),
Tools.makeShopExit("A kurt 'Bye' is all you hear as you leave", "Glenbrook")],
